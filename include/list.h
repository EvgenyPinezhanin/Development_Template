...
struct Link{
    ...
    Link(T val = T(), Link<T>* p = nullptr, Link<T>* n = nullptr) : value(val), prev(p), next(n) {};
};
...
listIterator<T> List<T>::erase(const listIterator<T>& pos) {
    ...
    if (pos.currLink == pastLast) throw logic_error("Iterator outside the list boundary");
...
{
    Link<T>* link = new Link<T>{l, last, pastLast};
    pastLast->prev = link;
    if (size == 0) {
        first = link;
        last = link;
...
ostream& operator<<(ostream &ostr, const List<T>& list)
{
    ...
    while (tmpLink != list.pastLast) {
        if (tmpLink->next != list.pastLast) cout << tmpLink->value << ", ";
